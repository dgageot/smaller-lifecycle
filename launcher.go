...
func (l *Launcher) Launch(self string, cmd []string) error {
	if err := l.env(); err != nil {
		return fmt.Errorf("modify env: %w", err)
	}
	process, err := l.processFor(cmd)
	if err != nil {
		return fmt.Errorf("determine start command: %w", err)
	}
	if err := os.Chdir(l.AppDir); err != nil {
		return fmt.Errorf("change to app directory: %w", err)
	}
	if process.Direct {
		...
		if err := l.Exec(binary,
			...
			return fmt.Errorf("direct exec: %w", err)
		}
		return nil
	}
	launcher, err := l.profileD()
	if err != nil {
		return fmt.Errorf("determine profile: %w", err)
	}
...
func (l *Launcher) env() error {
	...
	if err != nil {
		return fmt.Errorf("find app directory: %w", err)
	}
	return l.eachBuildpack(l.LayersDir, func(path string) error {
		...
		if err != nil {
			...
			return fmt.Errorf("find buildpack directory: %w", err)
		}
		...
		if err := eachDir(path, func(path string) error {
			...
			return fmt.Errorf("add layer root: %w", err)
		}
		if err := eachDir(path, func(path string) error {
			...
			return fmt.Errorf("add layer env: %w", err)
		}
		return nil
	})
}
